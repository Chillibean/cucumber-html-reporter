    ...
    var setStats = function (suite) {
        ...
        featureOutput.forEach(function (feature) {
            ...
            feature.elements.forEach(function (element, index) {
                ...
                if (!retriedScenarioTracker[element.id]) {
                    retriedScenarioTracker[element.id] = feature.elements.reduce((acc, curr, i) => {
                        if (curr.id === element.id) {
                            acc = {
                              scenarioIndex: i, 
                              passed: !curr.steps.find( step => step?.result?.status == result.status.failed)
                          }
                        }
                ...
                const currentTryIsFinal = retriedScenarioTracker[element.id].scenarioIndex === index;
                ...
                if (element.failed > 0 && !currentTryIsFinal) {
