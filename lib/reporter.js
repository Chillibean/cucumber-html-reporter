    ...
    var setStats = function (suite) {
        ...
        featureOutput.forEach(function (feature) {
            ...
            feature.elements.forEach(function (element, index) {
                ...
                element.timestamp = '';
                element.notes = '';
                element.retried = false

                if (element.type === 'background') {
                    return;
                }
                
                if (!retiredScenarioTracker[element.id]) {
                    const retriedScenarios = feature.elements.filter((featureElement, featureElementIndex) => {
                        if (featureElement.id == element.id) featureElement.index = featureElementIndex

                        return featureElement.id === element.id
                    } )
                    const finalTry = retriedScenarios[retriedScenarios.length-1]
                    const anyFailedStep = finalTry.steps.find( step => step?.result?.status == result.status.failed)
                    retiredScenarioTracker[finalTry.id] = {scenarioIndex: finalTry.index, passed: !anyFailedStep}
                }

                const finalTry = retiredScenarioTracker[element.id]
                const currentTryIsFinal = finalTry.scenarioIndex === index
                ...
                element.steps.forEach(function (step, count) {
                    if (step.embeddings !== undefined) {
                        ...
                        step.embeddings.forEach(function (embedding) {
                            ...
                            if (['text/plain', 'text/html', 'application/json'].includes(embeddingType)) {
                            ...
                            } else if (embeddingType === 'image/png' || embeddingType === 'image/link') {
                                var data;
                                let mimeType = "image/png;"
                                if(embeddingType === 'image/link') {
                                    var imagePath = path.join(screenshotsDirectory, embedding.data);
                                    if (path.parse(imagePath).ext === '.jpg') {
                                        // console.log(path.parse(imagePath).ext)
                                        mimeType = 'image/jpeg;'
                                    } 
                                ...
                                step.image[imgIndex] = 'data:' + mimeType + 'base64,' + data;
                                ...
                                if (options.storeScreenshots && options.storeScreenshots === true) {
                                    ...
                                    if (options.noInlineScreenshots) step.image[imgIndex] = path.relative(path.join(options.output, '..'), filename);
                                }
                                imgIndex++;
                            } else {
                ...
                if (element.failed > 0 && !finalTry.passed && !currentTryIsFinal) {
                    feature.scenarios.retried++;
                    element.retried = true;
                    return suite.scenarios.retried++;
                }
                ...
                if (element.failed > 0 && finalTry.passed) {
                    feature.scenarios.retried++;
                    element.retried = true;
                    return suite.scenarios.retried++;
                }
